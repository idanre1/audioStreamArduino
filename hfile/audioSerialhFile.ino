/*
Streaming Audio
https://github.com/kanderek/IDD-final-project/tree/master/streaming_audio
...
It uses a 1k serial revceive buffer for the audio samples, since
the ATMEGA328 only has 2k of RAM.  For chips with less RAM,
the Serial RX_BUFFER_SIZE variable can be reduced.
...
2.  Each time the host recieves a byte it sends the next
    xxx audio samples to fill the Arduino's receive buffer.
